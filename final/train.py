...
if __name__ == '__main__':
    ...
    # 데이터셋 탑재   - one hot encoding으로 변환
    (x_train, t_train), (x_test, t_test) = load_AReM(one_hot_label=True)

    # 모델 초기화    - lr을 인자로 추가
    network = Model(float(args.learning_rate), weight_decay_lambda=0, use_dropout=False, dropout_ration=0.15,
                    train_flg=True)

    # print(network.use_dropout)
    # 트레이너 초기화
    ...
    # print(network.train_flg)
    #
    # (_, _), (x_test, t_test) = load_AReM(one_hot_label=False)
    #
    # network2 = Model(float(args.learning_rate), weight_decay_lambda=0, use_dropout=True, dropout_ration=0.15,
    #                  train_flg=False)
    # network2.load_params("params.pkl")

    # print(network.predict(x_test))
    # print(network2.predict(x_test))
    # print(network.layers)
    # print(network2.layers)
    #
    # tester = Tester(network2, x_test, t_test)
